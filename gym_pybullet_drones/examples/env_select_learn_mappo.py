...
def create_env(env_type='multihover', multiagent=False, gui=False, record_video=False, num_drones=1, seed=None):
    """Create the appropriate environment based on env_type."""
    if multiagent:
        if env_type.lower() == 'flock':
            env = FlockAviary(
                gui=gui,
            ...
            print(f"✓ Created FlockAviary with {num_drones} drones")
        elif env_type.lower() == 'meetup':
            # MeetupAviary requires an even number of drones for pairing
            if num_drones % 2 != 0:
                print(f"⚠ Warning: MeetupAviary requires an even number of drones. Adjusting from {num_drones} to {num_drones + 1}")
            ...
            print(f"✓ Created MultiHoverAviary with {num_drones} drones")
...
def run(env_type=DEFAULT_ENV_TYPE,  # New parameter
    ...
    print(f"✓ Selected environment type: {env_type}")
    ...
    # Create output directory with environment type in name
    timestamp = datetime.now().strftime("%m.%d.%Y_%H.%M.%S")
    env_prefix = env_type.lower()
    filename = os.path.join(output_folder, f'mappo-{env_prefix}-{timestamp}')
    ...
    print("\n" + "="*60)
    print(f"MAPPO CONFIGURATION - {env_type.upper()} ENVIRONMENT:")
    print("="*60)
    print(f"  Environment: {env_type}")
    print(f"  Multi-agent: {multiagent}")
    print(f"  Number of agents: {num_drones}")
    ...
    # Training
    print("\n" + "="*60)
    print(f"STARTING MAPPO TRAINING - {env_type.upper()} ENVIRONMENT")
    print("="*60)
    ...
    try:
        ...
        if eval_results and 'ep_returns' in eval_results:
            ...
            print(f"\n" + "="*60)
            print(f"EVALUATION RESULTS - {env_type.upper()} ENVIRONMENT:")
            print("="*60)
    ...
    print("\n" + "="*60)
    print(f"RUNNING DEMONSTRATION - {env_type.upper()} ENVIRONMENT")
    print("="*60)
    ...
    print("\n" + "="*60)
    print(f"MAPPO TRAINING AND EVALUATION COMPLETED - {env_type.upper()} ENVIRONMENT!")
...
def test_environment_compatibility():
    ...
    try:
        ...
        env.close()
        print("✓ MultiHoverAviary compatible")
        
        print("\nTesting FlockAviary...")
        env = FlockAviary(gui=False, num_drones=2, obs=DEFAULT_OBS, act=DEFAULT_ACT)
        print(f"  Observation space: {env.observation_space}")
        print(f"  Action space: {env.action_space}")
        print(f"  Observation shape: {env.observation_space.shape}")
        print(f"  Action shape: {env.action_space.shape}")
        ...
        print(f"  Reward type: {type(reward)}, shape: {reward.shape if hasattr(reward, 'shape') else 'scalar'}")
        env.close()
        print("✓ FlockAviary compatible")
        
        print("\nTesting MeetupAviary...")
        # MeetupAviary requires even number of drones
        env = MeetupAviary(gui=False, num_drones=2, obs=DEFAULT_OBS, act=DEFAULT_ACT)
        print(f"  Observation space: {env.observation_space}")
        print(f"  Action space: {env.action_space}")
        print(f"  Observation shape: {env.observation_space.shape}")
        print(f"  Action shape: {env.action_space.shape}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env_type',           default=DEFAULT_ENV_TYPE,      type=str,           help='Environment type: "multihover", "flock", or "meetup" (default: "multihover")', metavar='')
    ...
    parser.add_argument('--num_drones',         default=DEFAULT_AGENTS,        type=int,           help='Number of drones for multi-agent (default: 3)', metavar='')
